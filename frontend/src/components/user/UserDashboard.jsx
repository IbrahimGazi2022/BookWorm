...
import { setDashboardData, setGoal } from "../../redux/slices/userSlice";
...
const UserDashboard = () => {
    ...
    const { recommendations, stats, goal } = useSelector((state) => state.user);
    ...
    const fetchData = async () => {
        try {
            ...
            dispatch(setGoal(goalData.readingGoal));
        ...
        <div className="min-h-screen bg-gray-50/50 p-4 md:p-8 lg:p-12">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
                    ...
                    <div className="lg:col-span-4 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col items-center justify-center text-center">
                        <div className="flex items-center justify-between w-full mb-6">
                            ...
                            <button onClick={handleOpenGoalModal} className="p-2 hover:bg-gray-50 rounded-xl transition-colors text-gray-400 cursor-pointer"><Edit2 className="w-4 h-4" /></button>
                        </div>
                        ...
                        {goal?.target > 0 ? (
                            ...
                            <div className="py-6 flex flex-col items-center">
                                ...
                                <button onClick={handleOpenGoalModal} className="bg-secondary text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-secondary/20 active:scale-95 transition-all cursor-pointer">Set 2026 Goal</button>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm mb-12">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <button onClick={() => navigate("/browse")} className="text-xs font-black text-secondary uppercase tracking-widest flex items-center gap-2 hover:gap-3 transition-all cursor-pointer">Explore More <ChevronRight className="w-4 h-4" /></button>
                    </div>
            ...
            {showGoalModal && (
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-[2.5rem] p-8 max-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-2xl font-black text-gray-800 tracking-tight">Set 2026 Goal</h2>
                            <button onClick={() => setShowGoalModal(false)} className="text-gray-400 hover:text-gray-600 cursor-pointer"><X className="w-6 h-6" /></button>
                        </div>
                        <div className="space-y-6">
                            ...
                            <button onClick={handleSaveGoal} className="w-full bg-secondary text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg shadow-secondary/20 active:scale-95 transition-all cursor-pointer">Lock Goal</button>
                        </div>
                    </div>
                </div>
            )}
